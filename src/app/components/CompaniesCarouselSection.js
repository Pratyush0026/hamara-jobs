...
const CompaniesCarouselSection = () => {
  ...
  <h2 className="text-3xl sm:text-4xl lg:text-[60px] font-extrabold leading-none text-[#001242]">
    Find jobs in India{"'"}s <span className="text-[#FF9046]">top</span> companies
        ...
        <div className="relative mt-32">
          {/* Carousel Track */}
          <div className="flex animate-scroll group-hover:animate-paused" onMouseEnter={(e) => e.currentTarget.style.animationPlayState = 'paused'} onMouseLeave={(e) => e.currentTarget.style.animationPlayState = 'running'}>
            {duplicatedCompanies.map((company, index) => (
              <div
                key={`${company}-${index}`}
                className="flex-shrink-0 mx-4 lg:mx-6"
